...
T64File::finalizeRead()
{
    ...
    if (noOfItems == 0) {
        ...
        if (noOfItems != noOfItemsStatedInHeader) {
            ...
            logme(LOG_WARN, "T64: Changing number of items from %ld to %ld.\n",
    ...
    for (i = 0; i < noOfItems; i++) {
        ...
        if (startAddrInContainer >= data.size) {
            logme(LOG_WARN, "T64: Offset mismatch. Sorry, can't repair.\n");
            return;
        }
        ...
        if (endAddrInMemory == 0xC3C6) {
            ...
            logme(LOG_WARN, "T64: Changing end address of item %ld from %04lX to %04lX.\n",
