...
Defaults::Defaults()
{
    ...
    setFallback(OPT_C64_SNAPSHOTS,              false);
    setFallback(OPT_C64_SNAPSHOT_DELAY,         10);
...
Defaults::load(const fs::path &path)
{
    ...
    if (!fs.is_open()) {
        throw Error(vc64::ERROR_FILE_NOT_FOUND);
    }
...
Defaults::load(std::stringstream &stream)
{
    ...
    {   SYNCHRONIZED
        ...
        while(std::getline(stream, input)) {
            ...
            throw Error(vc64::ERROR_SYNTAX, line);
        }
...
Defaults::getRaw(const string &key) const
{
    ...
    throw Error(vc64::ERROR_INVALID_KEY, key);
}
