 ...
 *  instruction */
typedef EmulatorException StateChangeException;
...
public:
    ...
    // Returns true if this functions is called from the emulator thread
    bool isEmulatorThread() const { return std::this_thread::get_id() == thread.get_id(); }

    // Returns true if this functions is called from the GUI thread
