...
static void filterTableInit(int sample_rate)
{
...
void SID6581_t::init()
{
    for (int v = 0; v < max_voices; v++)
    {
        ...
        uint8_t const mod_voi = ((v + max_voices) - 1) % max_voices;
        optr[v].modulator = &optr[mod_voi];
        optr[mod_voi].carrier = &optr[v];
        optr[v].filtVoiceMask = char(1 << v);
    }
...
void SID6581_t::port_w(int offset, int data)
{
    ...
    switch (offset)
    {
        ...
        case 0x17:
        case 0x18:
            // TODO: mixer_channel->update();
            reg[offset] = data;
            masterVolume = reg[0x18] & 15;
            masterVolumeAmplIndex = uint16_t(masterVolume << 8);
        ...
        default:
            ...
            if (offset < 7)
                optr[0].reg[offset] = uint8_t(data);
            else if (offset < 14)
                optr[1].reg[offset - 7] = uint8_t(data);
            else if (offset < 21)
                optr[2].reg[offset - 14] = uint8_t(data);
...
int SID6581_t::port_r(int offset)
{
