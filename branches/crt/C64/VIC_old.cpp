...
{	
	// do some stuff that has to be done to prepare a new rasterline
	setRasterline(line);
	// Check, if DMA lines are enabled or disabled (in line 30 only) (0x30! :)
	if (scanline == 0x30) { 
		dmaLinesEnabled = iomem[0x11] & 0x10; 
	}
	// Set the global dmaLine condition
	...
	// mark dma lines
	if (markDMALines && dmaLine) markLine(0, TOTAL_SCREEN_WIDTH, colors[LTRED]); 
	return true;
}
