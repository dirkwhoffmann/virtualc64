...
{
    if (debugger.breakpoints.isSetAt(addr)) throw AppError(Fault::BP_ALREADY_SET, addr);
...
{
    if (!debugger.breakpoints.guardNr(nr)) throw AppError(Fault::BP_NOT_FOUND, nr);
...
{
    if (!debugger.breakpoints.guardAt(addr)) throw AppError(Fault::BP_NOT_FOUND, addr);
...
{
    if (debugger.watchpoints.isSetAt(addr)) throw AppError(Fault::WP_ALREADY_SET, addr);
...
{
    if (!debugger.watchpoints.guardNr(nr)) throw AppError(Fault::WP_NOT_FOUND, nr);
...
{
    if (!debugger.watchpoints.guardAt(addr)) throw AppError(Fault::WP_NOT_FOUND, addr);
