...
Disassembler::disassI(u16 addr, u8 byte1, u8 byte2, u8 byte3, char *dst, isize tab) const
{
    ...
    switch (cpu.addressingMode[byte1]) {
        ...
        case AddrMode::ADDR_IMMEDIATE:    writer << Tab{} << Imm  { byte2 }; break;
        case AddrMode::ADDR_ZERO_PAGE:    writer << Tab{} << Zp   { byte2 }; break;
        case AddrMode::ADDR_ZERO_PAGE_X:  writer << Tab{} << Zpx  { byte2 }; break;
        case AddrMode::ADDR_ZERO_PAGE_Y:  writer << Tab{} << Zpy  { byte2 }; break;
        case AddrMode::ADDR_ABSOLUTE:     writer << Tab{} << Abs  { LO_HI(byte2, byte3) }; break;
        case AddrMode::ADDR_ABSOLUTE_X:   writer << Tab{} << Absx { LO_HI(byte2, byte3) }; break;
        case AddrMode::ADDR_ABSOLUTE_Y:   writer << Tab{} << Absy { LO_HI(byte2, byte3) }; break;
        case AddrMode::ADDR_DIRECT:       writer << Tab{} << Dir  { LO_HI(byte2, byte3) }; break;
        case AddrMode::ADDR_INDIRECT:     writer << Tab{} << Ind  { LO_HI(byte2, byte3) }; break;
        case AddrMode::ADDR_INDIRECT_X:   writer << Tab{} << Indx { byte2 }; break;
        case AddrMode::ADDR_INDIRECT_Y:   writer << Tab{} << Indy { byte2 }; break;
        case AddrMode::ADDR_RELATIVE:     writer << Tab{} << Rel  { (u16)(addr + 2 + (i8)byte2) }; break;
...
Disassembler::disassemble(char *str, u16 pc, u8 byte1, u8 byte2, u8 byte3) const
{
    ...
    switch (cpu.addressingMode[byte1]) {
        ...
        case AddrMode::ADDR_INDIRECT_X:   writer << Tab{} << Indx { byte2 }; break;
        case AddrMode::ADDR_INDIRECT_Y:   writer << Tab{} << Indy { byte2 }; break;
        case AddrMode::ADDR_RELATIVE:     writer << Tab{} << Rel  { (u16)(pc + 2 + (i8)byte2) }; break;
