...
{	
	...
	[[self document] setAlwaysWarp:NO];
	selectedSprite = 0;
	...
	switch (msg->id) {
		...
		case MSG_VC1541_DATA:
			...
			if (msg->i) {
				...
				if ([[self document] warpLoad])
					[c64 setWarpMode:YES];			
			} else {
				[driveBusy stopAnimation:self];
				[driveBusy setHidden:true];		
				if (![[self document]alwaysWarp])
					[c64 setWarpMode:NO];
			}			
			break;
...
- (IBAction)warpAction:(id)sender
{
	...
	bool warp = [[self document] alwaysWarp];
	[[self document] setAlwaysWarp:!warp];
	[c64 setWarpMode:warp];
	[self refresh];
}
